...
function signPad(confirmationText: string, exp: string, sig: string): string {
  return `<!doctype html>
<html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
...
<body>
  <div class="card">
    <div id="form">
      ...
      <p class="legal">${escapeHtml(confirmationText)}</p>
...
export function signatureRouter(): Router {
  ...
  router.get("/:jobId", async (req, res) => {
    ...
    return res.status(200).send(signPad(confirmationText, String(req.query.exp), String(req.query.sig)));
  });
