...
export const env = {
  ...
  // Secure by default. With a public Cloud Run URL and verification off, anyone who
  // knows the URL can POST a forged event asserting any driver's email and drive a job
  ...
  chatActionUrl: process.env.TMV_CHAT_ACTION_URL?.trim() || process.env.GOOGLE_CHAT_AUDIENCE?.trim() || "",
  ...
  /** Base URL Cloud Tasks calls back. Defaults to the origin of the Chat action URL. */
  workerBaseUrl:
    process.env.TMV_WORKER_BASE_URL?.trim().replace(/\/+$/, "") ||
    originOf(process.env.TMV_CHAT_ACTION_URL?.trim() || process.env.GOOGLE_CHAT_AUDIENCE?.trim() || ""),
