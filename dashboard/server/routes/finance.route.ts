...
export function financeRoute(): Router {
  ...
  router.get("/summary", async (req, res) => {
    try {
      ...
      let totalBase = pence(0);
      let totalExtras = pence(0);
      let totalOvertime = pence(0);
      let totalRevenue = pence(0);
      let totalCash = pence(0);
      let totalCard = pence(0);
      let totalBank = pence(0);
      let totalInvoice = pence(0);
      ...
      for (const j of jobs) {
        ...
        if (!j.reconciled && j.status === "COMPLETED") {
          ...
          unreconciledJobs.push({
            ...
            basePrice: toPounds(j.basePrice),
            extraCharges: toPounds(j.extraCharges),
            overtimeCharge: toPounds(j.overtimeCharge),
            totalCharges: toPounds(j.totalCharges),
      ...
      const timeline = [...timeSeriesMap.values()]
        ...
        .map(t => ({
          ...
          extras: Math.round(t.extras * 100) / 100,
          overtime: Math.round(t.overtime * 100) / 100,
          total: Math.round(t.total * 100) / 100
        }));

      return res.status(200).json({
        summary: {
          totalRevenuePounds: toPounds(totalRevenue),
          totalRevenueFormatted: formatGBP(totalRevenue),
          basePricePounds: toPounds(totalBase),
          extraChargesPounds: toPounds(totalExtras),
          overtimePounds: toPounds(totalOvertime),
          cashPounds: toPounds(totalCash),
          cardPounds: toPounds(totalCard),
          bankPounds: toPounds(totalBank),
          invoicePounds: toPounds(totalInvoice)
        },
        unreconciledJobs,
        timeline,
        meta: {
      ...
      return res.status(500).json({
        error: { code: "FINANCE_FETCH_FAILED", message: "Failed to fetch finance summary." }
      });
    }
  });
