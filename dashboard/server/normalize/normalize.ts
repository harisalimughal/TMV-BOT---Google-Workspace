  ...
  const checkCategory = (category: EvidenceCategory): EvidenceState => {
    ...
    if (matchingEvidence.length > 0) {
      ...
      if (status === "COMPLETED" && driveFileId) {
        items.push({
          ...
          thumbProxyUrl: `/admin/api/jobs/${encodeURIComponent(jobId)}/photos/${encodeURIComponent(driveFileId)}`,
    ...
    if (matchingPhoto && matchingPhoto[PHOTOS_MAP.fileId]) {
      ...
      items.push({
        ...
        thumbProxyUrl: `/admin/api/jobs/${encodeURIComponent(jobId)}/photos/${encodeURIComponent(fileId)}`,
