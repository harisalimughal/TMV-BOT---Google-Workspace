...
/**
 * Builds a valid binary PDF document (A4 page: 595 x 842 points)
 * representing the TMV Paper Job Report.
 */
export function generateJobPdf(job: NormalizedJob): Buffer {
  const contentLines: string[] = [];

  // Helper to add text stream instructions in PDF coordinates (0,0 is bottom-left)
  ...
  // --- Page Setup (A4: 595.28 x 841.89 pt) ---
  const left = 45;
  const right = 550;

  // Header Box
  addRect(left, 740, right - left, 65, "0.04 0.10 0.18", "0.04 0.10 0.18");
  addText("TMV OPERATIONS", left + 20, 775, 16, "/F2", "0.16 0.67 0.89"); // Cyan
  ...
  // Customer Confirmation & Evidence Audit Box
  ...
  // Footer Note
  addText("Generated by TMV Operations Dashboard (/ops) - Official Immutable Record", left + 80, 70, 8, "/F1", "0.5 0.58 0.68");
