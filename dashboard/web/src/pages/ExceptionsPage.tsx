...
export function ExceptionsPage({ onOpenJob }: Props) {
  ...
  if (isLoading) {
    return (
      <div className="py-20 text-center">
        ...
        <span className="text-[13px] font-medium text-muted">Auditing operational exception logs...</span>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="p-8 text-center text-status-red bg-paper rounded-xl border border-line shadow-sm hover:shadow-md transition">
        <AlertCircle className="w-6 h-6 mx-auto mb-2 text-status-red" />
        <h3 className="text-[12px] font-semibold text-ink">Failed to load exceptions</h3>
        <button
          onClick={() => refetch()}
          className="mt-3 px-3 py-1.5 rounded-lg bg-brand text-white text-[13px] font-medium hover:bg-brand-dark transition"
        >
          Retry
        </button>
      </div>
    ...
    <div className="space-y-6 max-w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          <div className="space-y-1">
            <div className="flex items-center gap-1.5 text-status-red text-[13px] font-bold uppercase tracking-wider">
          ...
          <span className="px-2 py-1 rounded-full bg-status-red text-white text-[13px] font-bold font-mono">
            P1
          </span>
        </div>
        ...
        >
          <div className="space-y-1">
            <div className="flex items-center gap-1.5 text-status-amber text-[13px] font-bold uppercase tracking-wider">
          ...
          <span className="px-2 py-1 rounded-full bg-status-amber text-white text-[13px] font-bold font-mono">
            P2
          </span>
        </div>
        ...
        >
          <div className="space-y-1">
            <div className="flex items-center gap-1.5 text-brand text-[13px] font-bold uppercase tracking-wider">
          ...
          <span className="px-2 py-1 rounded-full bg-brand text-white text-[13px] font-bold font-mono">
            P3
          </span>
        </div>
      </div>
      ...
      <div className="bg-paper p-4 rounded-lg border border-line flex flex-wrap items-center justify-between gap-3 shadow-sm hover:shadow-md transition">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* Category Pills */}
          <div className="flex flex-wrap items-center gap-1 p-0.5 bg-surface rounded-lg border border-line text-[13px]">
            <button
      ...
      <div className="bg-paper rounded-xl border border-line shadow-sm hover:shadow-md transition overflow-hidden">
        <div className="overflow-x-auto max-h-[calc(100vh-280px)]">
          <table className="w-full text-left text-[13px] border-collapse">
            <thead className="bg-surface/80 backdrop-blur-xs border-b border-line text-muted text-[12px] font-semibold sticky top-0 z-20">
              <tr className="h-10">
              ...
              {filteredItems.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-16 text-center text-muted">
                    ...
                    <p className="text-[12px] font-semibold text-ink">Zero exceptions found</p>
                    <p className="text-[13px] text-muted">All moves in this filter scope are running smoothly.</p>
                  </td>
                </tr>
              )}

              {filteredItems.map((ex: ExceptionItem) => (
                <tr key={ex.id} className="h-14 hover:bg-surface/80 transition">
                  ...
                  {/* Type */}
                  <td className="py-2.5 px-3 font-mono font-medium text-ink text-[13px] whitespace-nowrap ">
                    {ex.type}
                  </td>

                  {/* Job ID */}
                  <td className="py-2.5 px-3 font-mono font-bold text-brand text-[13px] whitespace-nowrap ">
                    {ex.jobId}
                  </td>

                  {/* Customer / Driver */}
                  <td className="py-2.5 px-3 ">
                    <span className="text-[13px] text-ink truncate block max-w-[200px]">{ex.customerName} &bull; <span className="text-muted font-mono font-medium">{ex.driverName}</span></span>
                  </td>

                  {/* Detail */}
                  <td className="py-2.5 px-3 text-ink-2 text-[13px]  max-w-sm truncate" title={ex.detail}>
                    {ex.detail}
                  </td>
                  ...
                  <td className="py-2.5 px-3 text-center whitespace-nowrap">
                    {ex.jobId !== "UNKNOWN" && onOpenJob ? (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg bg-surface hover:bg-brand hover:text-white text-brand text-[12px] font-semibold transition"
                      >
                        Inspect
                      </button>
                    ) : (
