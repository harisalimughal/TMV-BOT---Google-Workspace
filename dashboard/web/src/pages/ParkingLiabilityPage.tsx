...
export function ParkingLiabilityPage() {
  ...
  const tableHeader = (
    <>
      <th className="py-4 px-4 text-[12px] font-semibold text-muted uppercase tracking-[0.03em]">Driver</th>
      <th className="py-4 px-4 text-[12px] font-semibold text-muted uppercase tracking-[0.03em]">Date Submitted</th>
      <th className="py-4 px-4 text-[12px] font-semibold text-muted uppercase tracking-[0.03em]">Address</th>
      <th className="py-4 px-4 text-[12px] font-semibold text-muted uppercase tracking-[0.03em]">Full Client Name</th>
      <th className="py-4 px-4 text-[12px] font-semibold text-muted uppercase tracking-[0.03em]">Parking Restriction Photos</th>
      <th className="py-4 px-4 text-[12px] font-semibold text-muted uppercase tracking-[0.03em]">Signature</th>
    ...
    isLoading ? (
      ...
      <tr>
        <td colSpan={9} className="py-16 text-center">
          ...
          <h3 className="text-[15px] font-bold text-ink mb-1">No records match your filters</h3>
          ...
          >
            ...
            <td className="px-4">
              {invalidAddr ? (
                ...
                <span className="text-[14px] font-medium text-ink">{rawAddress}</span>
              )}
            </td>
            <td className="px-4">
              <span className={`text-[14px] font-medium ${clientName === 'Not Recorded' ? 'text-muted italic' : 'text-ink'}`}>
                {clientName}
              </span>
            </td>
            <td className="px-4">
              <div className="flex items-center gap-2">
                <div className="flex -space-x-2">
                  {Array.from({ length: Math.max(1, Math.min(3, photoCount)) }).map((_, i) => (
                    <div key={i} className="w-7 h-7 rounded-md bg-surface border border-line flex items-center justify-center overflow-hidden">
                      <div className="w-full h-full bg-black/5" />
                    </div>
                  ))}
                </div>
                <span className="text-[13px] font-medium text-muted">{photoCount} photos</span>
              </div>
            </td>
            <td className="px-4">
              {hasSig ? (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] bg-status-green-bg text-status-green text-[12px] font-semibold">
                  Signed
                </span>
              ) : (
                <span className="text-[13px] text-muted italic">Missing</span>
              )}
            </td>
