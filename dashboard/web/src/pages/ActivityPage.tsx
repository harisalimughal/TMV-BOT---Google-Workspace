        ...
        <div className="overflow-x-auto max-h-[calc(100vh-230px)]">
          <table className="w-full text-left text-[13px] border-collapse">
            ...
            <tbody className="divide-y divide-line bg-paper">
                ...
                <>
                  {[...Array(6)].map((_, i) => (
                    <tr key={i} className="h-14 animate-pulse">
                      ...
                      <td className="py-3 px-3 "><div className="w-20 h-4 bg-surface rounded" /></td>
                      <td className="py-3 px-3 "><div className="w-16 h-4 bg-surface rounded" /></td>
                      <td className="py-3 px-3 "><div className="w-24 h-5 bg-surface rounded-pill" /></td>
                      <td className="py-3 px-3 "><div className="w-28 h-4 bg-surface rounded" /></td>
              ...
              {!isLoading && data?.items.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-16 text-center text-muted">
                    ...
                    <p className="text-[12px] font-semibold text-ink">No activity records</p>
                    <p className="text-[13px] text-muted">No bot or field events recorded for this timeframe.</p>
                  </td>
                </tr>
              )}
              ...
              {!isLoading &&
                data?.items.map(act => (
                  <tr key={act.id} className="h-14 hover:bg-surface transition">
                    <td className="py-2.5 px-3 text-center ">
                      <input type="checkbox" className="rounded text-brand" />
                    </td>

                    <td className="py-2.5 px-3 font-mono text-ink-2 text-[11px] whitespace-nowrap " title={act.timestamp}>
                      {formatLondonDateTime(act.timestamp)}
                    </td>

                    <td className="py-2.5 px-3 font-mono font-semibold text-brand text-[13px] whitespace-nowrap ">
                      {act.jobId}
                    </td>
                    ...
                    <td className="py-2.5 px-3 text-ink-2 text-[13px] truncate max-w-sm" title={act.detail}>
                      {act.detail || "—"}
                    </td>
                  </tr>
                ))}
