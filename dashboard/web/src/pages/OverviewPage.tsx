  ...
  if (isLoading) {
    return (
      <div className="py-24 text-center">
        ...
        <p className="text-[14px] text-muted">Loading telemetry...</p>
      </div>
    );
  }
    ...
    <div className="max-w-[1440px] mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-2">
        <div className="space-y-1">
          <h2 className="text-[24px] font-bold text-ink tracking-tight">Analytics Overview</h2>
          <p className="text-[14px] text-muted">Real-time performance and financial metrics.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_12px_40px_rgb(0,0,0,0.06)] transition">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[16px] font-semibold text-ink">Gross Revenue</span>
                <Banknote className="w-5 h-5 text-muted" />
              </div>
              <div className="text-[44px] leading-none font-bold text-ink tracking-tighter mb-2">
                £{totalRevenue.toLocaleString("en-GB", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
              </div>
              <div className="flex items-center gap-2 text-[13px] font-medium text-status-green">
                <span className="px-2 py-0.5 rounded-full bg-status-green/10">+14.2%</span>
            ...
            <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_12px_40px_rgb(0,0,0,0.06)] transition">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[16px] font-semibold text-ink">Total Moves</span>
              ...
              <div className="flex items-center gap-2 text-[13px] font-medium">
                <span className="text-ink">{kpis.completed} delivered</span>
                <span className="text-muted font-normal">&bull; {kpis.inProgress} active</span>
              </div>
            </div>

            <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_12px_40px_rgb(0,0,0,0.06)] transition">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[16px] font-semibold text-ink">Completion Rate</span>
              ...
              <div className="flex items-center gap-2 text-[13px] font-medium text-status-green">
                <span className="px-2 py-0.5 rounded-full bg-status-green/10">Optimal</span>
                <span className="text-muted font-normal">SLA Target &gt;95%</span>
              </div>
            </div>

            <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_12px_40px_rgb(0,0,0,0.06)] transition">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[16px] font-semibold text-ink">Avg Arrival Delay</span>
                <Clock className="w-5 h-5 text-muted" />
              </div>
              <div className="text-[44px] leading-none font-bold text-ink tracking-tighter mb-2">
                {kpis.avgDelayMinutes}<span className="text-[20px] font-medium text-muted ml-1">min</span>
              </div>
              <div className="flex items-center gap-2 text-[13px] font-medium text-status-red">
                <span className="px-2 py-0.5 rounded-full bg-status-red/10">High Traffic</span>
                <span className="text-muted font-normal">Tol &lt;15m</span>
              </div>
            </div>
          ...
          {/* MAIN CHART CARD */}
          <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-[16px] font-semibold text-ink">Revenue Velocity</h3>
                <p className="text-[14px] text-muted mt-1">Daily billed move turnover</p>
              </div>
              <button className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-bg transition text-muted">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[16px] font-semibold text-ink">Top Drivers</h3>
              ...
              {charts.jobsByDriver.filter((d: any) => d.driverName !== 'Unassigned' && d.initials !== 'UN').slice(0, 5).map((d: any, i: number) => (
                <div key={i} className="flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[14px] font-semibold text-ink">{d.initials} Driver</div>
                      <div className="text-[12px] text-muted">{d.completed} delivered</div>
                    </div>
                  </div>
                  <div className="text-[14px] font-bold text-ink">{d.completed + d.active}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-paper p-8 rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[16px] font-semibold text-ink">Recent Activity</h3>
            ...
            <div className="space-y-6">
              {activityFeed.map((ex: any, i: number) => (
                <div key={ex.id || i} className="flex gap-4">
                  ...
                  <div>
                    <div className="text-[14px] text-ink leading-tight mb-1">
                      <span className="font-semibold">{ex.driverName}</span> logged <span className="font-medium text-status-red">{ex.type}</span>
                    </div>
                    <div className="text-[13px] text-muted line-clamp-1">{ex.detail}</div>
                    <div className="text-[12px] font-medium text-muted mt-2 tracking-wide uppercase">
              ...
              {activityFeed.length === 0 && (
                <div className="text-[14px] text-muted text-center py-4">No recent activity.</div>
              )}
            </div>
          </div>
