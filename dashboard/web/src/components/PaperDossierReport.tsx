  ...
  const Page = ({ page, totalPages, children }: { page: number, totalPages: number, children: React.ReactNode }) => (
    ...
    >
      ...
      <div className="pt-4 mt-auto border-t border-[#E5E7EB] flex justify-end shrink-0">
        <span className="text-[12px] font-semibold text-muted">{page}/{totalPages}</span>
      </div>
    </div>
  );
          ...
          <Page key={index} page={pageNum} totalPages={totalPages}>
            ...
            {pageNum === 1 && (
              <div className="mb-4 border border-[#E5E7EB] rounded-[12px] overflow-hidden shrink-0">
                <div className="flex items-center justify-between p-4 border-b border-[#E5E7EB] bg-white">
                  <span className="text-[13px] font-medium text-muted">Customer Name</span>
                  <span className="text-[13px] font-bold text-ink">{job.customerName || "N/A"}</span>
                </div>
                <div className="flex items-center justify-between p-4 border-b border-[#E5E7EB] bg-white">
                  <span className="text-[13px] font-medium text-muted">Pickup</span>
                  <span className="text-[13px] font-bold text-ink truncate max-w-[200px]">{job.pickup || "N/A"}</span>
                </div>
                <div className="flex items-center justify-between p-4 bg-white">
                  <span className="text-[13px] font-medium text-muted">Dropoff</span>
                  <span className="text-[13px] font-bold text-ink truncate max-w-[200px]">{job.dropoff || "N/A"}</span>
                </div>
              </div>
            )}
            ...
            {pageNum === 2 && (
              <div className="mb-4 border border-[#E5E7EB] rounded-[12px] overflow-hidden shrink-0">
                ...
                <div className="flex items-center justify-between p-4 border-b border-[#E5E7EB] bg-white">
                  <span className="text-[13px] font-medium text-muted">Payment Method</span>
                  <span className="text-[13px] font-bold text-ink">{job.paymentMethod || "Not recorded"}</span>
                </div>
              </div>
            )}
              ...
              <div className="mb-4 shrink-0">
                ...
                <div className="border border-[#E5E7EB] rounded-[12px] p-6 bg-[#F8F9FA] flex flex-col items-center justify-center min-h-[120px]">
                  {job.signatureUrl ? (
                    ...
                    <span className="text-[14px] font-semibold text-muted italic">Not captured</span>
                  )}
                  <span className="text-[11px] font-medium text-muted mt-4">Confirmed By: {job.clientConfirmedName || "N/A"}</span>
                  <span className="text-[11px] font-medium text-muted mt-1">Signed: {formattedTime}</span>
                </div>
              </div>
            )}
          </Page>
