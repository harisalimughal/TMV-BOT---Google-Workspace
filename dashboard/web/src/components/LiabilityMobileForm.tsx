  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      ...
      <div className="bg-[#F5F5F7] rounded-[40px] shadow-2xl w-full max-w-[390px] h-[844px] overflow-hidden flex flex-col relative border-[8px] border-black">
        ...
        {/* Header */}
        <div className="bg-white px-5 pb-4 pt-2 border-b border-line shrink-0 z-10 flex items-center justify-between">
          <h1 className="text-[20px] font-bold text-ink">Liability report</h1>
          <button onClick={onClose} className="text-[14px] font-medium text-brand hover:underline">Close</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-[#F5F5F5] flex flex-col relative pb-32">
          ...
          <div className="p-5">
             <div className="bg-white rounded-2xl p-5 shadow-sm border border-line mb-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-status-red-bg text-status-red flex items-center justify-center shrink-0">
                  <ShieldAlert className="w-6 h-6" />
                </div>
                <div>
                   <h2 className="text-[16px] font-bold text-ink">Liability for Damage</h2>
                   <p className="text-[13px] text-muted leading-tight mt-0.5">Please select all categories that apply to this notice.</p>
                </div>
             </div>
             ...
             <div className="sticky top-0 z-20 pb-4 pt-2 bg-[#F5F5F5]">
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full h-11 pl-10 pr-4 rounded-xl border border-line bg-white shadow-sm text-[15px] outline-none focus:border-brand transition"
                  />
                </div>
             </div>
             ...
             {selectedItems.size > 0 && (
                <div className="mb-4">
                  <h3 className="text-[12px] font-semibold text-muted uppercase tracking-wider mb-2">Selected ({selectedItems.size})</h3>
                  <div className="flex flex-wrap gap-2">
                    {Array.from(selectedItems).map(item => (
                      <span key={item} onClick={() => toggleItem(item)} className="px-3 py-1.5 rounded-full bg-brand-soft text-brand text-[13px] font-medium border border-brand/20 active:opacity-70 transition">
                        {item} &times;
                      </span>
                    ))}
                  </div>
             ...
             <div className="space-y-3">
               {filteredGroups.map(({ group, items }) => {
                 ...
                 return (
                   <div key={group} className="bg-white rounded-2xl shadow-sm border border-line overflow-hidden transition-all">
                     <button 
                       onClick={() => toggleGroup(group)}
                       className="w-full px-4 py-4 flex items-center justify-between text-left active:bg-surface transition"
                     >
                       <div className="flex items-center gap-2">
                         <h4 className="text-[15px] font-semibold text-ink">{group}</h4>
                         {selectedCount > 0 && (
                         ...
                         {items.map(item => {
                           ...
                           return (
                             <button
                               ...
                               className="w-full flex items-start gap-3 p-3 active:bg-surface rounded-xl transition text-left"
                             >
                               ...
                               <span className={`text-[15px] leading-snug ${isSelected ? "text-ink font-medium" : "text-ink-2"}`}>
                                 {item}
                               </span>
                             </button>
                           );
        ...
        <div className="absolute inset-x-0 bottom-0 bg-white border-t border-line px-5 pt-4 pb-8 flex flex-col gap-3 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-30">
          <button 
            disabled={selectedItems.size === 0}
            className="w-full h-14 rounded-2xl bg-[#2563EB] disabled:bg-surface disabled:text-muted disabled:border-line disabled:border hover:bg-blue-700 text-white text-[16px] font-bold shadow-sm transition flex items-center justify-center gap-2"
          >
