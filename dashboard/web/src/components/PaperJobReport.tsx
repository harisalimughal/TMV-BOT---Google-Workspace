...
export function PaperJobReport({ job, onClose }: Props) {
  ...
  const MetaRow = () => {
    ...
    return (
      <div className="flex items-start justify-between p-4 border border-line rounded-xl mb-6 bg-white shadow-sm">
        ...
        <div className="px-3 py-1 bg-surface text-[#1A1A1A] text-[13px] font-semibold rounded-lg border border-line">
          #{job.jobId}
        </div>
      </div>
    );
  ...
  return (
    <div className="paper-document bg-white font-sans text-[#1A1A1A]">
      ...
      {/* NO-PRINT CONTROLS */}
      <div className="no-print flex items-center justify-between p-4 mb-4 bg-surface rounded-xl border border-line">
        <div className="text-[14px] font-semibold text-ink">Preview Large-Format Report</div>
        <div className="flex items-center gap-3">
          <button onClick={() => window.print()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-line bg-white hover:bg-surface text-[13px] font-medium text-ink transition shadow-sm">
          ...
          {onClose && (
            <button onClick={onClose} className="px-3 py-1.5 text-[13px] font-medium text-muted hover:text-ink transition">
              Close
            </button>
          )}
        </div>
      ...
      {photos.map((photo, index) => (
        <div key={index} className="print-page-container">
          ...
          <div className="flex-1 flex flex-col mb-4">
            ...
            <div className="flex-1 flex items-center justify-center border border-line rounded-xl p-2 shadow-sm bg-[#FAFAFA]">
              <img 
                src={photo.url!} 
                alt={photo.label} 
      ...
      <div className="print-page-container">
        ...
        <div className="flex-1 flex flex-col gap-6 mt-4">
          ...
          <div className="bg-[#F7F7F7] rounded-xl border border-line p-6 shadow-sm">
          ...
          <div className="mt-4">
            ...
            <div className="w-full sm:w-1/2 h-[160px] border border-line rounded-xl bg-white p-4 shadow-sm flex items-center justify-center">
              {job.signatureUrl ? (
