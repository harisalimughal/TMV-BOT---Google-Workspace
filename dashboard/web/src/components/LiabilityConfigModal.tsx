  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[520px] flex flex-col max-h-[90vh] overflow-hidden">
        ...
        {/* Header */}
        <div className="px-6 py-4 border-b border-line flex items-center justify-between shrink-0 bg-white">
          <h2 className="text-[16px] font-bold text-ink">Manage Damage Categories</h2>
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 p-6 space-y-6 bg-[#FAFAFA]">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-[12px] font-semibold text-muted uppercase tracking-wider mb-1.5">Primary Form Type</label>
              <select className="w-full h-10 px-3 rounded-[8px] border border-line bg-white text-[13px] text-ink outline-none focus:border-brand shadow-sm">
                <option>Damage Liability & Assembly Risk Notice</option>
                <option>Waiver of Liability: Furniture Handling & Protection Etc</option>
              </select>
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-muted uppercase tracking-wider mb-1.5">Fallback Form Type</label>
              <select className="w-full h-10 px-3 rounded-[8px] border border-line bg-white text-[13px] text-ink outline-none focus:border-brand shadow-sm">
                <option>Waiver of Liability: Furniture Handling & Protection Etc</option>
                <option>Damage Liability & Assembly Risk Notice</option>
              </select>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[14px] font-bold text-ink">Category Items</h3>
              <div className="flex items-center gap-2">
                <button className="text-[12px] font-medium text-brand hover:text-brand-dark transition flex items-center gap-1">
            ...
            <div className="relative mb-6">
              ...
              <input 
                ...
                className="w-full h-9 pl-9 pr-3 rounded-full border border-line bg-white shadow-sm text-[13px] outline-none focus:border-brand transition"
              />
            </div>
            ...
            <div className="space-y-6">
              {filteredGroups.map(({ group, items }) => (
                <div key={group} className="space-y-2">
                  <h4 className="text-[12px] font-semibold text-muted uppercase tracking-wider">{group}</h4>
                  <div className="space-y-2">
                    {items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-2 group/row">
                        <div className="cursor-grab p-1.5 text-line-strong hover:text-muted transition">
                        ...
                        <input
                          ...
                          className="flex-1 h-10 px-3 rounded-[8px] border border-line bg-white shadow-sm text-[13px] text-ink outline-none focus:border-brand transition"
                          placeholder="Category name"
                        />
                        <button 
                          onClick={() => handleDeleteItem(group as keyof typeof INITIAL_CATEGORIES, idx)}
                          className="p-2 text-muted hover:text-status-red hover:bg-status-red-bg rounded-lg transition opacity-0 group-hover/row:opacity-100"
                        >
                    ...
                    <button 
                      ...
                      className="ml-8 flex items-center gap-1.5 text-[12px] font-semibold text-muted hover:text-brand transition py-1"
                    >
            ...
            <button className="w-full mt-6 py-3 border-2 border-dashed border-line-strong rounded-xl text-[13px] font-semibold text-muted hover:border-brand hover:text-brand transition flex items-center justify-center gap-2">
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-line bg-white flex items-center justify-end gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2 rounded-[8px] text-[13px] font-semibold text-muted hover:bg-surface hover:text-ink transition">
            Cancel
          </button>
