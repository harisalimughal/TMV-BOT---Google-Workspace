...
export function PhotoModal({ isOpen, onClose, title, photoUrl, driveUrl }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-900/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative max-w-4xl w-full bg-paper rounded-xl shadow-2xl border border-line overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-surface">
          <div>
            <h3 className="text-base font-bold text-ink">{title}</h3>
            <p className="text-xs text-muted">Evidence photograph proxy</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setZoomed(!zoomed)}
              className="p-2 rounded-lg hover:bg-surface-2 text-ink-2 transition"
            ...
            {driveUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="p-2 rounded-lg hover:bg-surface-2 text-tmv-blue transition"
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-lg hover:bg-status-red-bg text-status-red transition ml-2"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
