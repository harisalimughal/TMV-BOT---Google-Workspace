      ...
      <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-[600px] max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 border-b border-line flex items-center justify-between bg-[#FAFAFA] shrink-0">
          ...
          <button 
            ...
            className="p-2 -mr-2 text-muted hover:text-ink hover:bg-surface rounded-full transition disabled:opacity-50"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 min-h-0">
           {status === "success" ? (
             <div className="py-12 flex flex-col items-center justify-center text-center">
               ...
               <h3 className="text-[20px] font-bold text-ink mb-2">Report Generated!</h3>
               <p className="text-[14px] text-muted">
                 <span className="font-mono text-ink font-semibold">{generatedFilename}</span> downloaded.
               </p>
             </div>
           ) : status === "generating" ? (
             <div className="py-16 flex flex-col items-center justify-center text-center">
               <Loader2 className="w-10 h-10 text-brand animate-spin mb-4" />
               <h3 className="text-[16px] font-semibold text-ink">Generating {format} Report...</h3>
               <p className="text-[13px] text-muted mt-1">Compiling metrics and layout. Please wait.</p>
             </div>
           ) : (
             <div className="space-y-6">
                
                {status === "error" && (
                  <div className="p-4 bg-status-red/10 border border-status-red/20 rounded-xl flex items-start gap-3">
                    <AlertTriangle className="w-5 h-5 text-status-red shrink-0" />
                    <div>
                      <h4 className="text-[14px] font-bold text-status-red">Generation Failed</h4>
                      <p className="text-[13px] text-status-red/80 mt-0.5">Couldn't generate the report due to a timeout. Please try again.</p>
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {REPORT_TYPES.map(type => (
                      <label 
                        key={type}
                        className={`flex items-center gap-3 p-3 rounded-xl border transition cursor-pointer ${reportType === type ? 'border-brand bg-brand-soft/20 text-brand' : 'border-line bg-white hover:bg-surface text-ink'}`}
                      >
                        <input 
                          type="radio" 
                          name="reportType" 
                          ...
                          className="w-4 h-4 text-brand focus:ring-brand border-line" 
                        />
                ...
                {/* Date Range */}
                <div>
                  <label className="block text-[12px] font-semibold text-muted uppercase tracking-wider mb-3">Date Filter</label>
                  <DateRangePicker from={from} to={to} onChange={(f, t) => { setFrom(f); setTo(t); }} />
                  {noDatesSelected && (
                    <p className="text-[11px] text-status-red mt-2 font-medium flex items-center gap-1">
                ...
                {showDriverFilter && (
                   <div>
                     <label className="block text-[12px] font-semibold text-muted uppercase tracking-wider mb-2">Driver Filter</label>
                     <select 
                       value={driver}
                       onChange={e => setDriver(e.target.value)}
                       className="w-full h-10 px-3 rounded-xl border border-line bg-surface text-[13px] text-ink outline-none focus:border-brand transition"
                     >
                ...
                {/* Format Options */}
                <div>
                  <label className="block text-[12px] font-semibold text-muted uppercase tracking-wider mb-3">Export Format</label>
                  <div className="flex items-center gap-3">
                    <label className={`flex-1 flex flex-col items-center justify-center p-4 rounded-xl border transition cursor-pointer ${format === 'PDF' ? 'border-brand bg-brand-soft/20 text-brand' : 'border-line bg-white hover:bg-surface text-muted'}`}>
                       <input type="radio" className="sr-only" checked={format === 'PDF'} onChange={() => setFormat('PDF')} />
                       <FileDown className={`w-6 h-6 mb-2 ${format === 'PDF' ? 'text-brand' : 'text-muted'}`} />
                    ...
                    <label className={`flex-1 flex flex-col items-center justify-center p-4 rounded-xl border transition cursor-pointer ${format === 'CSV' ? 'border-brand bg-brand-soft/20 text-brand' : 'border-line bg-white hover:bg-surface text-muted'}`}>
                       <input type="radio" className="sr-only" checked={format === 'CSV'} onChange={() => setFormat('CSV')} />
                       <FileText className={`w-6 h-6 mb-2 ${format === 'CSV' ? 'text-brand' : 'text-muted'}`} />
        ...
        {status !== "success" && status !== "generating" && (
          <div className="px-6 py-4 border-t border-line bg-[#FAFAFA] flex items-center justify-end gap-3 shrink-0">
            ...
            <button 
              ...
              className="px-6 py-2 rounded-[12px] bg-ink disabled:opacity-50 disabled:cursor-not-allowed hover:bg-black text-white text-[13px] font-semibold shadow-sm transition flex items-center gap-2"
            >
